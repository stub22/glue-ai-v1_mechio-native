// RobotMotionTest.cpp : Defines the entry point for the console application.
//
...
{
	RobotProxy robot("source", "destination", "Avatar_ZenoR50", "127.0.0.1:5672");
	robot.initializeSender("robotAvatarZenoR50hostmotionFrame; {create: always, node: {type: queue}}");

	robot.setGoalPosition(NECK_YAW, 0.5);
	robot.setGoalPosition(NECK_ROLL, 0.5);
	robot.setGoalPosition(LEFT_SHOULDER_ROLL, 0.5);
	robot.setGoalPosition(RIGHT_SHOULDER_ROLL, 0.5);
	robot.moveAndWait(1000);

	for(int i=0; i<5; i++){
		robot.setGoalPosition(NECK_YAW, 0.2);
		robot.setGoalPosition(NECK_ROLL, 0.4);
		robot.setGoalPosition(LEFT_SHOULDER_ROLL, 0.6);
		robot.setGoalPosition(RIGHT_SHOULDER_ROLL, 0.8);
		robot.moveAndWait(2000);

		robot.setGoalPosition(NECK_YAW, 0.7);
		robot.setGoalPosition(NECK_ROLL, 0.5);
		robot.setGoalPosition(LEFT_SHOULDER_ROLL, 0.3);
		robot.setGoalPosition(RIGHT_SHOULDER_ROLL, 0.1);
		robot.moveAndWait(2000);
	}
	robot.setGoalPosition(NECK_YAW, 0.5);
	robot.setGoalPosition(NECK_ROLL, 0.5);
