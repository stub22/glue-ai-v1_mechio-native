...
int main(int argc, char **argv) {
    ...
    myStream->connect();
    MessageSender* sender = myStream->createSender("GyroscopeEvent; {create: always, node: {type: queue}}");
    HeaderRecord header;
    Vector3Record data;
    ...
    {
        cout << "Detected ST Micro gyro, initiating main loop" << endl;
    } else {
        mechio::common::error("This is NOT an ST Micro gyro, quitting");
        return 0;
    }
    free(resp);

    i2cSetDev(wr, 3);
    if (i2cWrite(wr, 2, 0x20, 0x0f)) {                            // power up, enable all axes
    ...
    {
        i2cSetDev(wr, 3);
        resp = i2cRead(wr, 0xa8, 6);                                // read 6 bytes for XYZ data
